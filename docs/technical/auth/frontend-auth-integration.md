...
В авторизованном лейауте ссылки на основные рабочие разделы (дашборд и последующие разделы) находятся в левой боковой колонке. Пункт «Проекты» (доменная сущность `branch`) является последним статичным пунктом; под ним отображается дерево активных проектов, планов и задач. При первом открытии загружается только корневой уровень, а дочерние узлы — по стрелке раскрытия соответствующего проекта или плана. Если маршрут дерева временно недоступен или возвращает устаревший `400/404`, фронтенд использует совместимый запрос списка веток, чтобы существующие проекты не исчезали. Ширина колонки регулируется пользователем перетаскиванием разделителя и сохраняется локально. Ссылки «Профиль» и «Настройки» размещаются в правом верхнем углу шапки рядом с аккаунтом пользователя. Архивирование проекта или задачи доступно только на странице открытой сущности.
