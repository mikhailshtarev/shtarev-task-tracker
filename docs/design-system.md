...
- Основные разделы приложения располагаются в постоянной боковой колонке слева. «Проекты» (доменная сущность `branch`) — последний статичный пункт этого списка независимо от количества остальных рабочих разделов.
- Ширина боковой колонки изменяется пользователем перетаскиванием вертикального разделителя в диапазоне 180–480 px и сохраняется между открытиями приложения. Разделитель доступен с клавиатуры.
- После пункта «Проекты» отображается управляемое дерево проектов, планов и задач в стиле Notion. Корневое действие «Новый проект» всегда доступно внизу дерева.
- У каждого проекта есть раскрытие/сворачивание и компактная кнопка «+» справа от названия. Кнопка открывает меню «Новый проект» и «Новая задача»; новый проект становится дочерним, а новая задача — дочерней текущего проекта.
- Планы работ также отображаются отдельными узлами дерева под родительской веткой. Задача, связанная с планом, отображается под планом; задача без плана — непосредственно под веткой.
...
#### 8.2.1. Дерево проектов, планов и задач

- Дерево показывает только активные сущности текущего пользователя; архивированные сущности исчезают после успешной операции.
- При открытии раздела загружается только корневой уровень проектов. Дочерние проекты, планы и задачи догружаются по нажатию стрелки раскрытия у соответствующего проекта или плана; повторное раскрытие использует уже загруженные данные до явного обновления.
- Вложенность проектов ограничена 7 уровнями. Отступ, иконка папки и состояние раскрытия должны однозначно показывать уровень.
- Действия создания доступны с клавиатуры: `Tab` переводит фокус на «+», `Enter` открывает меню, `Escape` закрывает его без создания.
- Архивирование проекта и задачи не выполняется из дерева. На странице открытой сущности в правом верхнем углу располагается кнопка «Архивировать» с подтверждением.
